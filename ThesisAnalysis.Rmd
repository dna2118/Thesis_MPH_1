...
```{r}
linearregression5SOBRO = lm(MCS12 ~ HousingSatisfactionScore + age + Income_Group + Race + Gender + Education_Group + housing_type + delayed_rent + moved_in_last_5_years + repairs_needed, data = SoBROData)
...
```{r}
joinedPCS12 = lm(pcs12 ~ housing_satisfaction_score + age + gender + race + income_group + education_group + delayed_rent + moved_in_last_5_years + repairs_needed + housing_type, data= Joined_data)
summary(joinedPCS12)
...
```{r}
joinedMCS12 = lm(mcs12 ~ housing_satisfaction_score + age + gender + race + income_group + education_group + delayed_rent + moved_in_last_5_years + repairs_needed + housing_type, data= Joined_data)
summary(joinedMCS12)
...
Joined_data %>%  
  ...
  summarize(n=n()) %>% 
  ggplot(aes(x = housing_satisfaction_score, y = mcs12, color = housing_type)) +
  geom_point(size = 1.5, alpha = 0.8) +
  geom_smooth(method =lm, color = "black", linetype = 1) +
  labs(
      x = "Housing Satisfaction Score",
...
#### Here, I've decided to measure the Cronbach's alpha for the housing satisfaction scale, since it was not a validated scale. Here, we find that the Cronbach's alpha for the Housing Satisfaction scale is 80.36%, demonstrating good internal consistency and reliability.
```{r}
library("psych")
...
```
```{r}
SmokingChi <- cbind(table(FinalSobroData$smoking_status),table(FinalBetancesData$smoking_status))
chisq.test(SmokingChi) %>% 
broom::tidy() %>% 
  knitr::kable(digits = 3)
```

```{r}
HousingSat.t.test = t.test(FinalBetancesData$housing_satisfaction_score, FinalSobroData$housing_satisfaction_score, var.equal = TRUE)


summary(HousingSat.t.test) 

HousingSat.t.test %>% 
  broom::tidy() %>%  
  select(estimate1, estimate2, statistic, p.value, conf.low, conf.high) %>% 
knitr::kable(col.names = c("Betances Mean RS Score", "SoBRO Mean RS Score", "t-statistic", "p-value", "Low CI", "High CI"), digits = 6)
```

```{r}
RentBurden <- cbind(table(FinalSobroData$delayed_rent),table(FinalBetancesData$delayed_rent))
chisq.test(RentBurden) %>% 
broom::tidy() %>% 
  knitr::kable(digits = 3)
```

```{r}
RepairsNeeded <- cbind(table(FinalSobroData$repairs_needed),table(FinalBetancesData$repairs_needed))
chisq.test(RepairsNeeded) %>% 
broom::tidy() %>% 
  knitr::kable(digits = 3)
```

```{r}
HousingInstability <- cbind(table(FinalSobroData$moved_in_last_5_years),table(FinalBetancesData$moved_in_last_5_years))
chisq.test(HousingInstability) %>% 
broom::tidy() %>% 
  knitr::kable(digits = 3)
```
